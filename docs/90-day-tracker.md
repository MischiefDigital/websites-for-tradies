...
| 3b | **GSC: /example-builds/ indexing** | ⬜ **Blair** | New indexable URL 31 Aug. Resubmit the sitemap (now 31 URLs) and request indexing for /example-builds/ — per the row-2 lesson, don't wait for Google to come back on its own. IndexNow already pinged Bing's side |
